...
switch ($act) {
    ...
    case "chinfo"://修改资料
        ...
        if (!preg_match('/^(((17[0-9]{1})|(13[0-9]{1})|(15[0,1,2,3,5,6,7,8,9]{1})|(18[0,1,2,3,5,6,7,8,9]{1}))+\d{8})$/', $phone)) {
    ...
    case "showuserinfo"://普通用户登录显示的主页面
        if (!isset($_SESSION['uName'])) outData(2, "你还没有权限");
        $uName = $_SESSION['uName'];
        ...
        outData(2, "操作错误,请重新登录");

    case "showveninfo"://厂商登录显示的主页面
        if (!isset($_SESSION['uName'])) outData(2, "你还没有权限");
        if (empty($_SESSION['uRole'])) outData(2, "你还没有权限");
        ...
        if ($rel) {
            if ($rel['Email'] == $useremail) {
                try {
                    ...
                    $mail->AddReplyTo("15112586572@163.com", "华会质量信息系统");//回复地址
                    $mail->From = "15112586572@163.com";
                    $mail->FromName = "华会质量信息系统";
                    $to = $useremail;
                    $mail->AddAddress($to);
...
//                    $mail->Body = "<html>请尽快点下面链接,时间(半小时内)过期则无效<br>http://" . $http . "/email_code.php?act=" . base64_encode($key) . "&ename=" . $ename . "&useremail=" . $useremail . "&time=" . $time . "</html>";
                    ...
                    $mail->Body = "<html>此邮件由华会云数据后台自动发送，用于用户重置密码，请勿回复。<br>点击下面的链接，或者复制下面的链接到浏览器地址栏打开。<br>http://" . $http . "/email_code.php?act=" . base64_encode($key) . "&ename=" . $ename . "&useremail=" . $useremail . "&time=" . $time . "<br>请尽快点击上面的链接,此链接30分钟内有效</html>";
//                    $mail->AltBody = "To view the message, please use an HTML compatible email viewer!"; //当邮件不支持html时备用显示，可以省略
    ...
    case "layout": //登录退出
        ...
        $name=$_SESSION["uName"];
        $db->update(DB_PREFIX . "usr", $tmp, "uName='" . $_SESSION["uName"] . "'","layout",$name);
        $_SESSION = array();
        session_destroy();
        //	outData(1,"退出成功");
    ...
    case "chcode": //修改密码
        if (empty($_SESSION['uName'])) outData(3, "你还没有登录");
        $uName = $_SESSION['uName'];
    ...
    case "authoreidt": //授权用户预留信息修改
        ...
        //	if($_SESSION['uRole']!=1) 	 outData(2,"你还没有权限");
        $VendID = $_POST['VendID'];
        if (empty($VendID)) outData(2, "操作有误,请重新登录");
    ...
    case "login":
        //登录时放SQL注入
        if (!get_magic_quotes_gpc()&&!empty($_POST)) // 判断magic_quotes_gpc是否为打开
        {
        ...
        foreach($_POST as $key=>$val){
            $_POST[$key] = str_replace("%", "＼%", $val); // 把' % '过滤掉
            $_POST[$key] = nl2br($val); // 回车转换
            $_POST[$key]= htmlspecialchars($val); // html标记转换
        }
		$loginbox = $_POST['loginbox'];
        $name = $_POST['username'];
        if (empty($name)) outData(2, "用户名不能为空");
        $password =trim($_POST['password']);
		if ($password=="") outData(2, "密码不能为空");
        if (!isset($password)) outData(2, "密码不能为空");
        ...
        if ($rel) {
            ...
            $db->update(DB_PREFIX . "usr", $tmp, "uName='" . $name . "'","login",$name);
            //根据角色的不同来跳转
    ...
    case "rusername":
        ...
        if (preg_match($pattern, $name)) {
            outData('2', '用户名不可用(请勿输入administrator开头的字符)');
        }
        $pattern = '/admin/i';
        if (preg_match($pattern, $name)) {
            outData('2', '用户名不可用(请勿输入admin开头的字符)');
        }
        $pattern = '/test/i';
        if (preg_match($pattern, $name)) {
            outData('2', '用户名不可用(请勿输入test开头的字符)');
        }
        $pattern = "/^[a-zA-Z]{1}([a-zA-Z0-9]|[._]){3,19}$/";
        if (!preg_match($pattern, $name)) {
            outData('2', '用户名请输入4-20位以字母开头且包含字母、数字、下划线的字符');
        }
    ...
    case "rpassword":
        ...
        if (!preg_match($pattern, $password)) {
            ...
            outData('2', '密码格式错误(请输入至少6位且包含字母和数字的字符)');
        }
        if (empty($rpassword)) {
            outData(1, "通过");
        } else if ($rpassword != "") {
...
//            $vename = $_POST['vename'];
//            if (empty($vename)) outData(2, "用户昵称不能为空");
//
...
//            $person = $_POST['person'];
//            if (empty($person)) outData(2, "用户公司不能为空");

            $phone = $_POST['phone'];
//            if (empty($phone)) outData(2, "座机号码不能为空");
            if(!empty($phone)){
                $isphone="/^([0-9]{3,4}-)?[0-9]{7,8}$/";
                if (!preg_match($isphone, $phone)) {
                    outData('2', '请输入正确的座机号码', '');
                }
                $oldrel2 = $db->find("select uName,uPwd,uRole from " . DB_PREFIX . "usr  where Mobile='" . $phone . "' limit 1 ");
                if ($oldrel2) outData(2, "该座机已被注册过了");
            }
            ...
            if ($oldrel3) outData(2, "该邮箱已被注册过了");
            $vendintro = $_POST['vendintro'];
            if (empty($vendintro)) outData(2, "用户手机不能为空");
            if (!preg_match('/^(((17[0-9]{1})|(13[0-9]{1})|(15[0,1,2,3,5,6,7,8,9]{1})|(18[0,1,2,3,5,6,7,8,9]{1}))+\d{8})$/', $vendintro)) {
                outData('2', '请输入正确的手机号码', '');
            }
//            $addre = $_POST['addre'];
//            if (empty($addre)) outData(2, "用户地址不能为空");
            $auth = $_POST['auth'];
            $arr = array(
                "uNick" => $vename,
                "Corp" => $person,
    ...
    case "useredit":    //后台管理员修改用户信息
        ...
        if ($role == 1) {
            ...
            $name = $_POST['rusername'];
            if (empty($name)) outData(2, "用户名不能为空");
//     勿删：用户修改暂时不做验证，如果用户修改做验证，现在系统的用户基本不满足要求
//            $pattern = '/administrator/i';
//            if (preg_match($pattern, $name)) {
//                outData('2', '用户名不可用');
//            }
...
//            if (empty($vename)) outData(2, "用户昵称不能为空");
            ...
            $department = trim($_POST['department']);
            if (empty($department)) outData(2, "用户所属部门不能为空");
...
//            if (empty($phone)) outData(2, "用户座机不能为空");
            if(!empty($phone)){
                $isphone="/^([0-9]{3,4}-)?[0-9]{7,8}$/";
                if (!preg_match($isphone, $phone)) {
                    outData('2', '请输入正确的座机号码', '');
...
//   产品管理
    ...
    case "productInfo":
       ...
       if($_SESSION['uRole'] == 0){
	    ...
	    if(isset($_SESSION['PDEL'])){ $PDEL=1; }else{ $PDEL=0; }
        if(isset($_SESSION['PMODIFY'])){ $PMODIFY=1; }else{ $PMODIFY=0; }
	  }
      foreach ($data as $k => $v) {
	    $data[$k]['PDEL'] = $PDEL;
		$data[$k]['PMODIFY'] = $PMODIFY;
	  }
        outData(1, $rel, $data);
        outData(2, "操作有误");
...
*/
    case "supplierAdd":
        ...
        if(!ctype_alnum($arr["Prefix"])){
            outData(2, "“Prefix”数据类型错误（请输入字母与数字的组合）");
        }
        //新增
        if(empty($_POST["Id"])){
            $arr["RegTime"]=date("Y-m-d H:i:s");
            //验证产品名是否重复
            $products = $db->findALL("select id from qis_supplierinfo where binary Supplier='" . $arr["Supplier"] . "'");
            if($products){
                outData(2, "供应商名称[".$arr["Supplier"]."]已经存在，请重新输入");
            }
            //验证Prefix是否重复
            $Prefix = $db->findALL("select id from qis_supplierinfo where binary Prefix='" . $arr["Prefix"] . "'");
            if($Prefix){
                outData(2, "Prefix[".$arr["Prefix"]."]已经存在，请重新输入");
            }
        ...
        //修改
            ...
            //验证Prefix是否重复
            $Prefix= $db->findALL("select count(1) as num  from qis_supplierinfo where Prefix='" . $arr["Prefix"] . "'");
            if($Prefix[0]['num']>0){
...
*/
    case "apncpnAdd":
        ...
        $arr["EEEECode"]=trim($_POST["EEEECode"]);
        if(!ctype_alnum($arr["APN"])){
            outData(2, "“APN”数据类型错误（请输入字母与数字的组合）");
        }
        if(!ctype_alnum($arr["CPN"])){
            outData(2, "“CPN”数据类型错误（请输入字母与数字的组合）");
        }
        if(!ctype_alnum($arr["EEEECode"])){
            outData(2, "“EEEECode”数据类型错误（请输入字母与数字的组合）");
        }
        //新增
        if(empty($_POST["Id"])){
            $arr["RegTime"]=date("Y-m-d H:i:s");
            //验证APN是否重复
            $products = $db->findALL("select id from qis_apncpninfo where binary APN='" . $arr["APN"] . "'");
            if($products){
                outData(2, "您新增的记录APN：[".$arr["APN"]."]已经存在，请重新输入");
            }
            //验证CPN是否重复
            $CPN= $db->findALL("select id from qis_apncpninfo where binary CPN='" . $arr["CPN"] . "'");
            if($CPN){
                outData(2, "CPN：[".$arr["CPN"]."]已经存在，请重新输入");
            }
            //验证EEEECode是否重复
            $EEEECode = $db->findALL("select id from qis_apncpninfo where binary EEEECode='" . $arr["EEEECode"] . "'");
            if($EEEECode){
                outData(2, "EEEECode：[".$arr["EEEECode"]."]已经存在，请重新输入");
            }
//            //验证EEEECode是否重复
...
*/
    ...
    case "getPrefix":
        ...
        if(!empty($_POST["Supplier"])){
            $data = $db->findAll("select *from qis_supplierinfo WHERE binary Supplier ='".$_POST["Supplier"]."'");
        }else{
            $data = $db->findAll("select *from qis_supplierinfo ");
        }
        outData(1,"",$data);
        outData(2, "操作有误");
    ...
    case "getSupplier":
        ...
        if(!empty($_POST["Prefix"])){
            $data = $db->findAll("select *from qis_supplierinfo WHERE binary Prefix ='".$_POST["Prefix"]."'");
        }else{
            $data = $db->findAll("select *from qis_supplierinfo ");
        }
        outData(1,"",$data);
    ...
    ////根据选择的APN获取CPN/EEEECode
    case "getCPN_EEEECode":
        if (!isset($_SESSION['uName'])) outData(2, "你还没有权限");
        if(!empty($_POST["APN"])){
            $data = $db->findAll("select *from qis_apncpninfo WHERE binary APN ='".$_POST["APN"]."'");
        }else{
            $data = $db->findAll("select *from qis_apncpninfo ");
        }

        outData(1,"",$data);
        outData(2, "操作有误");

    ////根据选择的CPN获取APN/EEEECode
    case "getAPN_EEEECode":
        if (!isset($_SESSION['uName'])) outData(2, "你还没有权限");
        if(!empty($_POST["CPN"])){
            $data = $db->findAll("select *from qis_apncpninfo WHERE binary CPN ='".$_POST["CPN"]."'");
        }else{
            $data = $db->findAll("select *from qis_apncpninfo");
        }
        outData(1,"",$data);
    ...
    ////根据选择的EEEECode获取APN/CPN
    case "getAPN_CPN":
        if (!isset($_SESSION['uName'])) outData(2, "你还没有权限");
        if(!empty($_POST["EEEECode"])){
            $data = $db->findAll("select *from qis_apncpninfo WHERE binary EEEECode ='".$_POST["EEEECode"]."'");
        }else{
            $data = $db->findAll("select *from qis_apncpninfo");
        }
...
//                验证数据类型
                if(!ctype_alnum($val[2])){
                    outData(2, "导入的EXCEL表中第".($key)."行“Prefix”数据类型错误（请输入数字和字母的组合）");
                }
                if(!ctype_alnum($val[3])){
                    outData(2, "导入的EXCEL表中第".($key)."行“EEEECode”数据类型错误（请输入数字和字母的组合）");
                }
                if(!ctype_alnum($val[4])){
                    outData(2, "导入的EXCEL表中第".($key)."行“APN”数据类型错误（数字或字母）");
                }
                if(!ctype_alnum($val[5])){
                    outData(2, "导入的EXCEL表中第".($key)."行“CPN”数据类型错误（请输入数字和字母的组合）");
                }

                //验证导入的产品名称是否是存在的，存在则通过，不存在则返回
                $product= $db->findAll("select Id from qis_productinfo  where Products ='".$val[0]."'" );
                if(count($product)<1){
                    outData(2, "导入的EXCEL表中第".($key)."行“产品名”不存在，请检查您的EXCEL表格");
                }
                //验证导入的供应商是否是存在的，存在则通过，不存在则返回
                $product= $db->findAll("select Id from qis_supplierinfo  where Supplier ='".$val[1]."'and Prefix='".$val[2]."'" );
                if(count($product)<1){
                    outData(2, "导入的EXCEL表中第".($key)."行“供应商/Prefix”不存在，请检查您的EXCEL表格");
                }
                //验证导入的APN/CPN是否是存在的，存在则通过，不存在则返回
                $product= $db->findAll("select Id from qis_apncpninfo  where APN ='".$val[4]."'and CPN='".$val[5]."'and EEEECode='".$val[3]."'" );
                if(count($product)<1){
                    outData(2, "导入的EXCEL表中第".($key)."行“APN/CPN/EEEECode”不存在，请检查您的EXCEL表格");
                }
            }
...
*/
    case "getOnlineUsr"://企业用户所有用户资料信息
        ...
        //if ($_SESSION['uRole'] != 1) outData(2, "你还没有权限");
        $rel = array();
        $sql = "select count(1) as num from " . DB_PREFIX . "usr where  uStatusWeb='1'";
        $sql1 = "select * from " . DB_PREFIX . "usr where  uStatusWeb='1' ";
        $rel = $db->findALL($sql);
        $pagepar = 10;
        $rel[1]['sum'] = ceil($rel[0]['num'] / $pagepar);
...
//                    }
            ...
            if ($model) {
                $sql .= "and  model='" . $trunk[$model] . "'";
                $sql1 .= "and model='" . $trunk[$model] . "'";
            }
            if ($begintime) {
...
//                $rel = mysql_query('INSERT INTO com_dbbackup (path,posttime,editor,db) VALUES ("' . $path . '","' . time() . '","' . $_SESSION["uName"] . '","common")');
            $dbmanage = new DBManage ('localhost', 'root', 'HuaHui@111111', 'common', 'utf8');
...
//        print_r($data);
    ...
    case "showThisTpl":
        ...
        if (isset($_POST["id"])) {
            ...
            $data = $db->find($sql);
            $data["tplcontent"]=stripslashes($data["tplcontent"]);
//        print_r($data);
            outData(1, "", $data);
        }
        outData(2, "操作有误");
...
//                }
        ...
        $savePath = ROOT_PATH . '/external/uploads/upfile/tmp/img/';
        /*以时间来命名上传的文件*/
        $str = date('Ymdhis');
        $file_name = $str . "." . $file_type;
        /*是否上传成功*/
    ...
    case "UsrAuthority":
        ...
        if (!isset($_SESSION['uName'])) outData(2, "你还没有权限");
        //if(empty($_SESSION['uRole']))  outData(2,"你还没有权限");
        $_GET['uName']=str_replace(' ', '', $_GET['uName']);
        $role = isset($_GET['role']) ? $_GET['role'] : 0;
        $sql = "select count(1) as num from " . DB_PREFIX . "usr where uRole !=0";
        if (isset($_GET['uName']) && trim($_GET['uName']) != null) {
            $sql = "select count(1) as num from " . DB_PREFIX . "usr where uRole !=0 and uName like '%" . trim($_GET['uName']) . "%'";
        }
//        print_r($sql);
//        exit;
        $rel = $db->findALL($sql);
        ...
        if (isset($_GET['uName']) && trim($_GET['uName']) != null) {
            $sql1 = "select * from " . DB_PREFIX . "usr where uRole !=0 and uName like '%" . trim($_GET['uName']) . "%' limit " . $lim;
        }
    ...
    case "Application":
        ...
        foreach ($data as $k => $v) {
            ...
            $funcs = array();//功能数组
            //根据用户的ID从common.usrbusiness表中获取该用户所有的业务、模块、功能信息
            $datauser = $db->find("select Id from " . DB_PREFIX . "usr  where  uName='" . $v['UsrName'] . "'  limit 1 ");
            $business = $db->find("select BusinessInfo from " . DB_PREFIX . "usrbusiness  where  UsrId='" . $datauser['Id'] . "'  limit 1 ");
            if ($business) {
                $data1 = json_decode($business["BusinessInfo"], true);    //json转数组
                if ($data1) {
                    if ($data1) {
                    ...
                    foreach ($modules as $key => $func) {
                        foreach ($modules2 as $key2 => $func2) {
						    if($func==$func2){
							    unset($modules[$key]);//删除数组同值元素
							}
						}
...
//					$rel=$db->delete(DB_PREFIX."vendauthor","Id=".$venid);
    ...
    case "AuthorityDel":
        ...
        if ($_SESSION['uRole'] == 0) {
            ...
            if (empty($userId)) outData(2, "操作有误");
            $rel = $db->query("update " . DB_PREFIX . "usrbusiness set CountInfo='',BusinessInfo='' where UsrId='" . $userId . "'");
			$rel = $db->query("update " . DB_PREFIX . "businesspower set BusinessInfo='' where UsrId='" . $userId . "'");
            if ($rel) outData(1, "撤销成功");
            outData(2, "撤销失败");
   ...
   case "Warning"://业务预警列表
        ...
        if($_SESSION['uRole'] == 0){
        if (isset($_GET['uName']) && trim($_GET['uName']) != null) {
            $rel = $db->findALL("select count(1) as num from " . DB_PREFIX ."collectwarn  where Alias like '%" . trim($_GET['uName']) . "%'");
        }else{
            $rel = $db->findALL("select count(1) as num from " . DB_PREFIX ."collectwarn");
        }
        }else{
			if (isset($_GET['uName']) && trim($_GET['uName']) != null) {
			$rel = $db->findALL("select count(1) as num from " . DB_PREFIX ."collectwarn where UsrName like '%".$_SESSION['uName']."%' and Alias like '%" . trim($_GET['uName']) . "%' ");
            $rel = $db->findALL("select count(1) as num from " . DB_PREFIX ."collectwarn  where Alias like '%" . trim($_GET['uName']) . "%'");
              }else{
            $rel = $db->findALL("select count(1) as num from " . DB_PREFIX ."collectwarn");
            }
        ...
        if($_SESSION['uRole'] == 0){
        if (isset($_GET['uName']) && trim($_GET['uName']) != null) {
             $data = $db->findAll("select * from " . DB_PREFIX . "collectwarn where Alias like '%" . trim($_GET['uName']) . "%'  limit " . $lim);
        }else{
             $data = $db->findAll("select * from " . DB_PREFIX . "collectwarn  limit " . $lim);
        }
        }else{
		if (isset($_GET['uName']) && trim($_GET['uName']) != null) {
             $data = $db->findAll("select * from " . DB_PREFIX . "collectwarn where Alias like '%" . trim($_GET['uName']) . "%' and UsrName like '%".$_SESSION['uName']."%'  limit " . $lim);
        }else{
             //$data = $db->findAll("select * from " . DB_PREFIX . "collectwarn  limit " . $lim);
             $data = $db->findAll("select * from " . DB_PREFIX . "collectwarn where UsrName like '%".$_SESSION['uName']."%'  limit " . $lim);
        }
    ...
    case "indexWarnning"://首页获取业务预警列表
        if (!isset($_SESSION['uName'])) outData(2, "你还没有权限");

            $data = $db->findAll("select * from " . DB_PREFIX . "collectwarn  order by RegTime desc limit 0,6");

        outData(1, 1, $data);
    case "indexEmail"://首页获取邮件提醒
        if (!isset($_SESSION['uName'])) outData(2, "你还没有权限");

            $sql="select * from ".DB_PREFIX."emailpush where AuditStatus=0 order by PushDate DESC limit 0,6 ";
            $data = $db->findAll($sql);

        outData(1, 1, $data);

    case "indexCallboard"://首页获取公告板
        if (!isset($_SESSION['uName'])) outData(2, "你还没有权限");
    ...
    case "indexReview"://首页获取审核管理
        if (!isset($_SESSION['uName'])) outData(2, "你还没有权限");
        if($_SESSION['uRole'] == 0){
            $sql = "select * from " . DB_PREFIX . "application limit 0,6";
            $data = $db->findAll($sql);
            foreach ($data as $k => $v) {
 ...
 case "Warnlist"://分析预警列表
        ...
        if($_SESSION['uRole'] == 0){
        if (isset($_GET['uName']) && trim($_GET['uName']) != null) {
            $rel = $db->findALL("select count(1) as num from " . DB_PREFIX ."analysiswarn  where Alias like '%" . trim($_GET['uName']) . "%'");
        }else{
        ...
        if($_SESSION['uRole'] == 0){
        if (isset($_GET['uName']) && trim($_GET['uName']) != null) {
             $data = $db->findAll("select * from " . DB_PREFIX . "analysiswarn where Alias like '%" . trim($_GET['uName']) . "%'  limit " . $lim);
        }else{
             $data = $db->findAll("select * from " . DB_PREFIX . "analysiswarn   limit " . $lim);
        }
        }else{
		if (isset($_GET['uName']) && trim($_GET['uName']) != null) {
             $data = $db->findAll("select * from " . DB_PREFIX . "analysiswarn where Alias like '%" . trim($_GET['uName']) . "%' and UsrName like '%".$_SESSION['uName']."%'  limit " . $lim);
        }else{
             //$data = $db->findAll("select * from " . DB_PREFIX . "collectwarn  limit " . $lim);
             $data = $db->findAll("select * from " . DB_PREFIX . "analysiswarn where UsrName like '%".$_SESSION['uName']."%' limit " . $lim);
        }
        ...
        foreach ($data as $k => $v) {
			...
			$data[$k]['AnalysisStatus']=$data2['Status'];
            $data[$k]['AnalysisStatus'] = isset($data[$k]['AnalysisStatus']) ? $data[$k]['AnalysisStatus'] : 0;
        }
        //权限分配
	  if($_SESSION['uRole'] == 0){
	    $YDEL=1; $YMODIFY=1;
	  }else{
	    if(isset($_SESSION['YDEL'])){ $YDEL=1; }else{ $YDEL=0; }
        if(isset($_SESSION['YMODIFY'])){ $YMODIFY=1; }else{ $YMODIFY=0; }
	  }
      foreach ($data as $k => $v) {
	    $data[$k]['YDEL'] = $YDEL;
		$data[$k]['YMODIFY'] = $YMODIFY;
	  }
        outData(1, $rel, $data);

  case "Warnliststatus"://分析预警弹出框
     if (!isset($_SESSION['uName'])) outData(2, "你还没有权限");
      $data = $db->find("select * from ".DB_PREFIX."analysiswarn  where Id=".$_GET['id']."");
      $info=$data['Alias'];
     outData(2,$info);


    case "getEmailAlias"://写邮件获取策略名
        if (!isset($_SESSION['uName'])) outData(2, "你还没有权限");
        $sql= "select * from ".DB_PREFIX."analysisresult where 1=1";
//        $sql1 = "select * from ".DB_PREFIX."analysisresult where Products='".$_POST['Products']."'and Supplier='".$_POST['Supplier']."'and SubSupplier='".$_POST['SubSupplier']."'and Config='".$_POST['Config']."'and APN='".$_POST['APN']."'and CPN='".$_POST['CPN']."'and StartTime >='".$_POST['StartTime']."'and StopTime <='".$_POST['StopTime']."'";
        if(!empty($_POST["Products"])){
            $sql.= " and Products='".$_POST['Products']."'";
        }
        if(!empty($_POST["Supplier"])){
            $sql.= "and Supplier='".$_POST['Supplier']."'";
        }
        if(!empty($_POST["SubSupplier"])){
            $sql.= "and SubSupplier='".$_POST['SubSupplier']."'";
        }
        if(!empty($_POST["Config"])){
            $sql.= "and Config='".$_POST['Config']."'";
        }
        if(!empty($_POST["APN"])){
            $sql.= "and APN='".$_POST['APN']."'";
        }
        if(!empty($_POST["CPN"])){
            $sql.= "and CPN='".$_POST['CPN']."'";
        }
        if(!empty($_POST["StartTime"])){
            $sql.= "and StartTime >='".$_POST['StartTime']."'";
        }
        if(!empty($_POST["StopTime"])){
            $sql.= "and StopTime <='".$_POST['StopTime']."'";
        }
        $data="";
...
//        print_r($data);exit;
  ...
  //业务策略增加和修改
   ...
   case "WarningInfo"://业务预警提示信息列表
        ...
        //if (empty($_SESSION['uRole'])) outData(2, "你还没有权限");
        //if ($_SESSION['uRole'] != 0) outData(2, "你还没有权限");
//       $rel = $db->findALL("select count(1) as num from " . DB_PREFIX ."collectwarn  where Alias like '%" . $_GET['uName'] . "%'");
         if(isset($_GET['y'])){ $y=$_GET['y']; if($y==0){ $w=''; }  if($y==1){ $w=' and Status=0 '; }  if($y==2){ $w=' and Status=1 '; } if($y==3){ $w=' and Status=2 '; } }
         if ($_SESSION['uRole'] == 0){
            if(isset($_GET['y'])){
                //模糊查询
                if (isset($_GET['uName']) && trim($_GET['uName']) != null) {
                    $rel = $db->findALL("select count(1) as num from " . DB_PREFIX ."warninglist  where Alias like '%" . trim($_GET['uName']) . "%' ".$w."");
                }else{
                ...
                //模糊查询
                if (isset($_GET['uName']) && trim($_GET['uName']) != null) {
                    $rel = $db->findALL("select count(1) as num from " . DB_PREFIX ."warninglist  where Alias like '%" . trim($_GET['uName']) . "%'");
                }else{
...
//               $rel = $db->findALL("select count(1) as num from " . DB_PREFIX ."warninglist where 1=1");
            ...
            if(isset($_GET['y'])){
                //模糊查询
                if (isset($_GET['uName']) && trim($_GET['uName']) != null) {
                    $rel = $db->findALL("select count(1) as num from " . DB_PREFIX ."warninglist where 1=1 and Status!=2 and Alias like '%" . trim($_GET['uName']) . "%' ".$w."  and UsrName like '%".$_SESSION['uName']."%' ".$w." ");
                }else{
                    $rel = $db->findALL("select count(1) as num from " . DB_PREFIX ."warninglist where 1=1 and Status!=2 and UsrName like '%".$_SESSION['uName']."%' ".$w." ");
                }
//              $rel = $db->findALL("select count(1) as num from " . DB_PREFIX ."warninglist where 1=1 and Status!=2 and UsrName like '%".$_SESSION['uName']."%' ".$w." ");
            }else{
                //模糊查询
...
//                    $rel = $db->findALL("select count(1) as num from " . DB_PREFIX ."warninglist where 1=1 and Status!=2 and UsrName like '%".$_SESSION['uName']."%'  ".$w." ");
        ...
        if ($_SESSION['uRole'] == 0){
            if(isset($_GET['y'])){
                //模糊查询
                if (isset($_GET['uName']) && trim($_GET['uName']) != null) {
                    $data = $db->findALL("select  * from " . DB_PREFIX ."warninglist  where Alias like '%" . trim($_GET['uName']) . "%' ".$w." order by RegTime desc limit ". $lim);
                }else{
                    $data = $db->findAll("select * from " . DB_PREFIX . "warninglist where 1=1 ".$w." order by RegTime desc limit " . $lim);
                }
//                $data = $db->findAll("select * from " . DB_PREFIX . "warninglist where 1=1 ".$w." limit " . $lim);
                 }else{
                    //模糊查询
                    if (isset($_GET['uName']) && trim($_GET['uName']) != null) {
                        $data = $db->findALL("select  * from " . DB_PREFIX ."warninglist  where Alias like '%" . trim($_GET['uName']) . "%' order by RegTime desc  limit ". $lim);
                    }else{
                        $data = $db->findAll("select * from " . DB_PREFIX . "warninglist where 1=1 order by RegTime desc limit " . $lim);
                    }
//                    $data = $db->findAll("select * from " . DB_PREFIX . "warninglist where 1=1 limit " . $lim);
             ...
             if(isset($_GET['y'])){
                 //模糊查询
                 if (isset($_GET['uName']) && trim($_GET['uName']) != null) {
                     $data = $db->findAll("select * from " . DB_PREFIX . "warninglist where 1=1 and Status!=2  and Alias like '%" . trim($_GET['uName']) . "%' and  UsrName like '%".$_SESSION['uName']."%' ".$w."  order by RegTime desc limit " . $lim);
//                     $rel = $db->findALL("select  * from " . DB_PREFIX ."warninglist  where Alias like '%" . $_GET['uName'] . "%' ".$w." limit". $lim);
                 }else{
                     $data = $db->findAll("select * from " . DB_PREFIX . "warninglist where 1=1 and Status!=2 and  UsrName like '%".$_SESSION['uName']."%' ".$w."  order by RegTime desc limit " . $lim);
                 }
//                $data = $db->findAll("select * from " . DB_PREFIX . "warninglist where 1=1 and Status!=2 and  UsrName like '%".$_SESSION['uName']."%' ".$w."  limit " . $lim);
              }else{
                 //模糊查询
                 if (isset($_GET['uName']) && trim($_GET['uName']) != null) {
                     $data = $db->findAll("select * from " . DB_PREFIX . "warninglist where 1=1 and Status!=2 and Alias like '%" . trim($_GET['uName']) . "%' and  UsrName like '%".$_SESSION['uName']."%'  order by RegTime desc limit " . $lim);
                 }else{
                     $data = $db->findAll("select * from " . DB_PREFIX . "warninglist where 1=1 and Status!=2 and  UsrName like '%".$_SESSION['uName']."%'  order by RegTime desc limit " . $lim);
                 }
//                 $data = $db->findAll("select * from " . DB_PREFIX . "warninglist where 1=1 and Status!=2 and  UsrName like '%".$_SESSION['uName']."%'  limit " . $lim);
                }
 ...
 case "WarnInfo"://分析预警提示信息列表
     ...
     if ($_SESSION['uRole'] == 0){
         if(isset($_GET['y'])){
             //模糊查询
             if (isset($_GET['uName']) && trim($_GET['uName']) != null) {
                 $rel = $db->findALL("select count(1) as num from " . DB_PREFIX ."analysiswarninfo  where Alias like '%" . trim($_GET['uName']) . "%' ".$w."");
             }else{
...
//             exit;
             //模糊查询
             if (isset($_GET['uName']) && trim($_GET['uName']) != null) {
                 $rel = $db->findALL("select count(1) as num from " . DB_PREFIX ."analysiswarninfo  where Alias like '%" . trim($_GET['uName']) . "%'");
             }else{
...
//               $rel = $db->findALL("select count(1) as num from " . DB_PREFIX ."warninglist where 1=1");
         ...
         if(isset($_GET['y'])){
             //模糊查询
             if (isset($_GET['uName']) && trim($_GET['uName']) != null) {
                 $rel = $db->findALL("select count(1) as num from " . DB_PREFIX ."analysiswarninfo where 1=1 and Status!=2 and Alias like '%" . trim($_GET['uName']) . "%' ".$w."  and UsrName like '%".$_SESSION['uName']."%' ".$w." ");
             }else{
                 $rel = $db->findALL("select count(1) as num from " . DB_PREFIX ."analysiswarninfo where 1=1 and Status!=2 and UsrName like '%".$_SESSION['uName']."%' ".$w." ");
             }
//              $rel = $db->findALL("select count(1) as num from " . DB_PREFIX ."warninglist where 1=1 and Status!=2 and UsrName like '%".$_SESSION['uName']."%' ".$w." ");
         }else{
             //模糊查询
     ...
     if ($_SESSION['uRole'] == 0){
         if(isset($_GET['y'])){
             ...
             if (isset($_GET['uName']) && trim($_GET['uName']) != null) {
                 ...
                 $data = $db->findALL("select  * from " . DB_PREFIX ."analysiswarninfo  where Alias like '%" . trim($_GET['uName']) . "%' ".$w." order by RegTime desc limit". $lim);
             }else{
                 $data = $db->findAll("select * from " . DB_PREFIX ."analysiswarninfo where 1=1 ".$w." order by RegTime desc limit " . $lim);
             }
//                $data = $db->findAll("select * from " . DB_PREFIX . "warninglist where 1=1 ".$w." limit " . $lim);
         }else{
             ...
             //模糊查询
             if (isset($_GET['uName']) && trim($_GET['uName']) != null) {
                 $data = $db->findALL("select  * from " . DB_PREFIX ."analysiswarninfo  where Alias like '%" . trim($_GET['uName']) . "%'order by RegTime desc limit ". $lim);
             }else{
                 $data = $db->findAll("select * from " . DB_PREFIX ."analysiswarninfo where 1=1 order by RegTime desc limit  " . $lim);
             }
//                    $data = $db->findAll("select * from " . DB_PREFIX . "warninglist where 1=1 limit " . $lim);
         }
         ...
         if(isset($_GET['y'])){
             //模糊查询
             if (isset($_GET['uName']) && trim($_GET['uName']) != null) {
                 $data = $db->findAll("select * from " . DB_PREFIX . "analysiswarninfo where 1=1 and Status!=2  and Alias like '%" . trim($_GET['uName']) . "%' and  UsrName like '%".$_SESSION['uName']."%' ".$w." order by RegTime desc limit " . $lim);
//                     $rel = $db->findALL("select  * from " . DB_PREFIX ."warninglist  where Alias like '%" . $_GET['uName'] . "%' ".$w." limit". $lim);
             }else{
                 $data = $db->findAll("select * from " . DB_PREFIX . "analysiswarninfo where 1=1 and Status!=2 and  UsrName like '%".$_SESSION['uName']."%' ".$w." order by RegTime desc limit " . $lim);
             }
//                $data = $db->findAll("select * from " . DB_PREFIX . "warninglist where 1=1 and Status!=2 and  UsrName like '%".$_SESSION['uName']."%' ".$w."  limit " . $lim);
         }else{
             //模糊查询
             if (isset($_GET['uName']) && trim($_GET['uName']) != null) {
                 $data = $db->findAll("select * from " . DB_PREFIX . "analysiswarninfo where 1=1 and Status!=2 and Alias like '%" . trim($_GET['uName']) . "%' and  UsrName like '%".$_SESSION['uName']."%' order by RegTime desc  limit " . $lim);
             }else{
                 $data = $db->findAll("select * from " . DB_PREFIX . "analysiswarninfo where 1=1 and Status!=2 and  UsrName like '%".$_SESSION['uName']."%' order by RegTime desc limit " . $lim);
             }
//                 $data = $db->findAll("select * from " . DB_PREFIX . "warninglist where 1=1 and Status!=2 and  UsrName like '%".$_SESSION['uName']."%'  limit " . $lim);
         }
   ...
   case "Kanbanset":
	    ...
	    if(isset($_SESSION['LDEL'])){ $LDEL=1; }else{ $LDEL=0; }
        if(isset($_SESSION['LMODIFY'])){ $LMODIFY=1; }else{ $LMODIFY=0; }
	  }
      foreach ($data as $k => $v) {

		//if(isset($_SESSION["'".$v['IpAddress']."'"])){
			//if($_SESSION["'".$v['IpAddress']."'"]<time()){
			 // $data[$k]['uStatusBoard']=0;
		//	}else{
			//  $data[$k]['uStatusBoard']=1;
		//	}
		// }else{ $data[$k]['uStatusBoard']=0; }   //这个是通过SESSION来判断但是放在服务器没法正常使用

        if($v['uStatusBoard']<time()){
		  $data[$k]['uStatusBoard']=0;
		}else{
		  $data[$k]['uStatusBoard']=1;
		}
   ...
   case "Analysislist":
        ...
        $Alias=isset($_GET['uName']) ? trim($_GET['uName']) : "";
         if($Alias){
       ...
       // }
        ...
        //if($_SESSION['uRole'] == 0){
             ...
             $data = $db->findAll("select * from " . DB_PREFIX . "analysis where 1=1 and Type!=2 ".$Alias.$Status.$Type." order by RegTime desc limit " . $lim);
    ...
    case "DataType":
         ...
         $product = trim($_POST['product']);
         $db1 = new Mysql();
         $db1->connect(DB_HOST,DB_USER,DB_PWD,'qis',DB_CHARSET,1);
         $rel = $db1->find("select * from qis_productinfo  where Products='". $product."' and DataType=1 ");
        if ($rel) {
            outData(1, "成功");
        }else{
            outData(3, "失败");
    ...
    case "AnalysisTime":
       ...
       // }else{
          $rel = $db->find("select * from " . DB_PREFIX . "analysis  where Alias='".$alias."' and Status='0' limit 1 ");
      //  }
...
//        print_r($_POST);
//        exit;
        $pieces["tplcontent"] =addslashes($_POST["html"]);
        $pieces["tplname"] = trim($_POST["tplname"]);
        $result = $db->findAll("select * from " .DB_PREFIX."emailtpl where tplname='".$pieces["tplname"]."'");
        if(!empty($result)){
            outData(2, "模板名称已存在，请重新输入");
        }
        if (empty($pieces["tplname"] )) outData(2, "请输入邮件模板名称", 2);
        $pieces["creattime"] = date("Y-m-d H:i:s");
        $pieces["creator"] = $_SESSION['uName'];
        $rel = $db->save(DB_PREFIX . "emailtpl", $pieces);
        if ($rel) outData(1, "新增模板成功", $rel);
        outData(2, "新增模板失败");
...
//        exit;
        $analysis = $_POST['analysis'];
        $relm = $db->find("select * from " . DB_PREFIX . "analysis where Type!=2 and Alias='".$analysis."'");

        $analysis=$relm["ConditionInfo"];
        //outData(2, $analysis);
        $data = $db->findAll("select * from " .DB_PREFIX."template where ConditionInfo='".$analysis."'");
        if($data){
            outData(1,888, $data);
    ...
    case "Reportlist":
        ...
        if (!isset($_SESSION['uName'])) outData(2, "你还没有权限");
        $rel = $db->findALL("select count(1) as num from " . DB_PREFIX . "template");
        ...
        $data = $db->findAll("select * from " . DB_PREFIX . "template limit " . $lim);
        //权限分配
		 if($_SESSION['uRole'] == 0){
	   $LDEL=1; $LMODIFY=1;
	  }else{
    ...
    case "Reportadd":
		...
		$ConditionInfo = trim($_POST['ConditionInfo']);
		if (empty($TemplateName)) outData(2, "请填写模板的名称！");
		if (empty($Addr)) outData(2, "请填写模板的地址！");
        $HtmlAddr='models/'.$Addr;
        $arr = array(
        ...
        $rel = $db->save(DB_PREFIX . "template", $arr);
   ...
   case "Summarylist":
    ...
    case "AnalysisWarn":
        ...
        if (empty($defaultvalue)) outData(2, "请填写临界值！");
        ...
        if($_POST['id']==0){
		$relw2 = $db->find("select * from ".DB_PREFIX."analysiswarn where Alias='".$Analysislist."'");
		}else{
		$relw2 = $db->find("select * from ".DB_PREFIX."analysiswarn where Id!=".$_POST['id']." and Alias='".$Analysislist."'");
		}
		if($relw2){  outData(2, "该分析策略已经被使用！"); }
   ...
   case "showpeo"://显示联系人
       ...
       if ($curren == $rel[1]['sum']) {
        ...
        if($_SESSION['uRole']==0){
            if(isset($_POST["str"])&&$_POST["str"]==2){
                $data=$db->findAll("select * from ".DB_PREFIX."emailcontact order by id desc ");
            }else{
            ...
            if(isset($_POST["str"])&&$_POST["str"]==2){
                $data=$db->findAll("select * from ".DB_PREFIX."emailcontact where UsrName='".$name."' order by id desc");
            }else{
...
//        exit;
   ...
   case "sendup"://提交申请
       $arr=array(
           ...
           "cont1" =>addslashes($_POST['cont1']),
           "cont2" =>addslashes($_POST['cont2']),
           "analysisResult" =>addslashes($_POST['analysisResult'])
       );


       $_POST["ContentId"]=json_encode($arr);
        $name=$_SESSION['uName'];
        if(!$name) outData(2,"还没有登录");
       $_POST["ReceAddr"]=trim($_POST["ReceAddr"]);
        ...
        //  $arr['ContentId']=str_replace("<img src=\"","<img src=\"".$ho,$arr['ContentId']);
            ...
            if($_POST['str']==2){
                    ...
                    try {
                        ...
                        //$mail->IsSendmail(); //如果没有sendmail组件就注释掉，否则出现“Could  not execute: /var/qmail/bin/sendmail ”的错误提示
                        $mail->AddReplyTo($sender['Account'], "华会质量信息系统");//回复地址
...
//					"Corp"=>$_POST['Corp'],
    ...
    case "qisUsrAdd":
        ...
        if ($_POST) {
            ...
            if (!preg_match('/^(((17[0-9]{1})|(13[0-9]{1})|(15[0,1,2,3,5,6,7,8,9]{1})|(18[0,1,2,3,5,6,7,8,9]{1}))+\d{8})$/', $data['Mobile'])) {
                outData('2', '请输入正确的手机号码', '');
            }
